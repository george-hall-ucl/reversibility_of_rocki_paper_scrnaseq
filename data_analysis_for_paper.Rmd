...
```{r}
skin_data.with_outliers <- skin_data
...
reducedDim(ctrl6d_cells.sce, "UMAP")[,2] <- (-1) * reducedDims(ctrl6d_cells.sce)$UMAP[,2]
...
sshot_6d <- slingshot(d6_cells.sce, Idents(d6_cells),
                      reducedDim = "HARMONY", start.clus = "Basal")
sshot_12d <- slingshot(d12_cells.sce, Idents(d12_cells),
                       reducedDim = "HARMONY", start.clus = "Basal")
...
```{r}
skin_data.with_outliers <- skin_data.with_outliers %>% SCTransform() %>%
                           CellCycleScoring(s.features = s.genes,
                                            g2m.features = g2m.genes)
skin_data.with_outliers$CC.difference <- skin_data.with_outliers$S.Score - skin_data.with_outliers$G2M.Score
skin_data.with_outliers <- skin_data.with_outliers %>%
                           SCTransform(vars.to.regress = c("CC.difference")) %>%
                           RunPCA(features = skin_data.with_outliers@assays$SCT@var.features,
                                  npcs = 50)
skin_data.with_outliers$donor_idents <- sapply(skin_data.with_outliers$orig.ident,
                                                         function(x) if (x %in% c("1","2","3","4")) {"1"}
                                                         else if (x %in% c("5","6","7","8")) {"2"}
                                                         else {"3"})
skin_data.with_outliers <- skin_data.with_outliers %>%
...
p <- FeaturePlot(skin_data.with_outliers, "VIM")
if (save_images) {
