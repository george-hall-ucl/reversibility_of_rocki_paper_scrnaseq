...
vlnplot_limits <- function(data, metric, ylab, limits) {
    df <- data.frame(x = Idents(data), y = data$metric)
...
pass_qc <- c()
for (j in seq_len(length(skin_data$orig.ident))) {
    i <- as.integer(as.character(skin_data$orig.ident[j]))
    pass_qc <- c(pass_qc, all(skin_data$nFeature_RNA[j] >= nfeature_df$lower[i],
                              skin_data$nFeature_RNA[j] <= nfeature_df$upper[i],
                              skin_data$nCount_RNA[j] >= ncount_df$lower[i],
                              skin_data$nCount_RNA[j] <= ncount_df$upper[i],
                              skin_data$percent.mt[j] >= pct_mito_df$lower[i],
                              skin_data$percent.mt[j] <= pct_mito_df$upper[i]))
}
...
```{r}
skin_data_with_outliers <- skin_data
...
skin_data <- subset(skin_data,
                    ...
                    invert = TRUE)
# Remove fibroblasts
skin_data <- subset(skin_data, subset = ACTG2 > 1 | DLK1 > 1, invert = TRUE)
```
...
skin_data_outliers_removed <- skin_data
```
...
```{r}
s_genes <- cc.genes$s_genes
g2m_genes <- cc.genes$g2m_genes
skin_data <- CellCycleScoring(skin_data, s.features = s_genes,
                              g2m.features = g2m_genes)
skin_data$CC.difference <- skin_data$S.Score - skin_data$G2M.Score
skin_data <- SCTransform(skin_data, vars.to.regress = c("CC.difference"))
skin_data_cell_cycle_regressed <- skin_data
```
...
donor_idents <- sapply(skin_data$orig.ident,
                       function(x) {
                           if (x %in% c("1", "2", "3", "4")) {
                               "1"
...
skin_data$donor_idents <- donor_idents

skin_data <- skin_data %>% RunHarmony("donor_idents", assay.use = "SCT")
skin_data_harmonised <- skin_data

sample_class <- sapply(skin_data$orig.ident,
                       function(x) {
                           if (x %in% c("1", "5", "9")) {
                               "Control-6D"
...
skin_data$sample_class <- sample_class
skin_data$sample_class <- factor(skin_data$sample_class,
                                           levels = c("Control-6D", "ROCKi-6D",
                                                      "Control-12D",
                                                      "ROCKi-12D"))

treatment_class <- sapply(skin_data$sample_class,
                       function(x) {
...
skin_data$treatment_class <- treatment_class
```
...
```{r}
daseq_out_6d <- getDAcells(Embeddings(object = d6_cells, reduction = "harmony"),
                           cell.labels = as.character(d6_cells$sample_class),
                           labels.1 = c("Control-6D"),
                           labels.2 = c("ROCKi-6D"),
                           plot.embedding = Embeddings(object = d6_cells,
                                                       reduction = "umap"))

daseq_out_12d <- getDAcells(Embeddings(object = d12_cells,
                                       reduction = "harmony"),
                            cell.labels = as.character(d12_cells$sample_class),
                            labels.1 = c("Control-12D"),
                            labels.2 = c("ROCKi-12D"),
                            plot.embedding = Embeddings(object = d12_cells,
                                                        reduction = "umap"))
...
num_da_cells_6d <- length(c(daseq_out_6d$da.down, daseq_out_6d$da.up))
proportion_da_6d <- num_da_cells_6d / length(colnames(d6_cells))
num_da_cells_12d <- length(c(daseq_out_12d$da.down, daseq_out_12d$da.up))
proportion_da_12d <- num_da_cells_12d / length(colnames(d12_cells))
```
...
```{r out.width = "100%"}
Idents(skin_data) <- skin_data$sample_class
...
vol_6d_legend <- ggplot(data.frame(class = as.character(seq_len(length(labels))),
                                   x = seq_len(length(labels))),
...
skin_data$cell_type_pred <- SCINA(as.matrix(GetAssayData(skin_data)),
                                  signatures)$cell_labels
d6_cells$cell_type_pred <- SCINA(as.matrix(GetAssayData(d6_cells)),
                                 signatures)$cell_labels
d12_cells$cell_type_pred <- SCINA(as.matrix(GetAssayData(d12_cells)),
                                  signatures)$cell_labels

d6_cells$cell_type_pred <- factor(d6_cells$cell_type_pred,
                                  levels = c("Basal", "TA", "TD", "unknown"))
d12_cells$cell_type_pred <- factor(d12_cells$cell_type_pred,
                                   levels = c("Basal", "TA", "TD", "unknown"))
...
cell_type_df_6d <- data.frame(CellType = d6_cells$cell_type_pred,
                           Class = d6_cells$sample_class)
...
cell_type_prop_graph_6d <- cell_type_prop_graph(cell_type_df_6d,
                                                c("Control-6D ", "ROCKi-6D "))
(cell_type_prop_graph_6d_no_legend <- cell_type_prop_graph_6d +
                                      theme(legend.position = "none"))

if (save_images) {
    save_tiff(cell_type_prop_graph_6d_no_legend, "fig_6e.tiff", width = 6,
              height = 6)
}

cell_type_df_12d <- data.frame(CellType = d12_cells$cell_type_pred,
                           Class = d12_cells$sample_class)
cell_type_df_12d$Class <- factor(cell_type_df_12d$Class,
                                 levels = c("Control-6D", "ROCKi-6D",
                                            "Control-12D", "ROCKi-12D"))
cell_type_prop_graph_12d <- cell_type_prop_graph(cell_type_df_12d,
                                                 c("Control-6D 6D ",
                                                   "ROCKi-6D 6D "))
(cell_type_prop_graph_12d_no_legend <- cell_type_prop_graph_12d +
                                       theme(legend.position = "none"))
if (save_images) {
    save_tiff(cell_type_prop_graph_12d_no_legend, "fig_6f.tiff", width = 6,
              height = 6)
}
```
...
obs_total_prop_diff_6d <- calc_cell_type_props(d6_cells$cell_type_pred,
                                               d6_cells$treatment_class)
obs_total_prop_diff_12d <- calc_cell_type_props(d12_cells$cell_type_pred,
                                                d12_cells$treatment_class)
...
for (r in 1:10000) {
    ...
    c6d_idx <- sample(which(d6_cells$treatment_class == "Control"),
                      length(which(d6_cells$treatment_class == "Control")),
                      replace = TRUE)
    r6d_idx <- sample(which(d6_cells$treatment_class == "ROCKi"),
                      length(which(d6_cells$treatment_class == "ROCKi")) - 1,
                      replace = TRUE)
    d6_idx <- c(c6d_idx, r6d_idx)

    c12d_idx <- sample(which(d12_cells$treatment_class == "Control"),
                      length(which(d12_cells$treatment_class == "Control")),
                      replace = TRUE)
    r12d_idx <- sample(which(d12_cells$treatment_class == "ROCKi"),
                      length(which(d12_cells$treatment_class == "ROCKi")) - 1,
                      replace = TRUE)
    d12_idx <- c(c12d_idx, r12d_idx)

    # Find cell type and treatment class labels for this resample
    d6_cell_type <- d6_cells$cell_type_pred[d6_idx]
    d6_treat_class <- d6_cells$treatment_class[d6_idx]

    d12_cell_type <- d12_cells$cell_type_pred[d12_idx]
    d12_treat_class <- d12_cells$treatment_class[d12_idx]
...
skin_data$sample_class_space <- skin_data$sample_class
levels(skin_data$sample_class_space) <- c("Control-6D ",
                                                    "ROCKi-6D ",
                                                    "Control-6D 6D ",
                                                    "ROCKi-6D 6D ")
...
prepare_cells_for_slingshot <- function(cells) {
    ...
    if (cells$sample_class == "Control-6D") {
    cells <- RenameIdents(object = cells, "0" = "TA", "1" = "Basal", "2" = "TD")
    } else {
...
d6_cells_sce <- as.SingleCellExperiment(d6_cells)
d12_cells_sce <- as.SingleCellExperiment(d12_cells)
ctrl6d_cells_sce <- as.SingleCellExperiment(ctrl6d_cells)
rocki6d_cells_sce <- as.SingleCellExperiment(rocki6d_cells)
ctrl12d_cells_sce <- as.SingleCellExperiment(ctrl12d_cells)
rocki12d_cells_sce <- as.SingleCellExperiment(rocki12d_cells)

# Change UMAP values to make figures all similarly oriented
reducedDim(ctrl6d_cells_sce, "UMAP")[, 2] <- (-1) * reducedDims(ctrl6d_cells_sce)$UMAP[, 2]
reducedDim(rocki6d_cells_sce, "UMAP")[, 2] <- (-1) * reducedDims(rocki6d_cells_sce)$UMAP[, 2]
reducedDim(ctrl12d_cells_sce, "UMAP")[, 1] <- (-1) * reducedDims(ctrl12d_cells_sce)$UMAP[, 1]

sshot_6d <- slingshot(d6_cells_sce, Idents(d6_cells),
                      reducedDim = "HARMONY", start.clus = "Basal")
sshot_12d <- slingshot(d12_cells_sce, Idents(d12_cells),
                       reducedDim = "HARMONY", start.clus = "Basal")

sshot_ctrl6d_separate <- slingshot(ctrl6d_cells_sce,
                                   Idents(ctrl6d_cells),
                                   reducedDim = "UMAP", start.clus = "Basal")
sshot_rocki6d_separate <- slingshot(rocki6d_cells_sce,
                                    Idents(rocki6d_cells),
                                    reducedDim = "UMAP", start.clus = "Basal")
sshot_ctrl12d_separate <- slingshot(ctrl12d_cells_sce,
                                    Idents(ctrl12d_cells),
                                    reducedDim = "UMAP", start.clus = "Basal")
sshot_rocki12d_separate <- slingshot(rocki12d_cells_sce,
                                     Idents(rocki12d_cells),
                                     reducedDim = "UMAP", start.clus = "Basal")

umap_curve_6d <- slingCurves(embedCurves(sshot_6d,
                                         reducedDims(d6_cells_sce)$UMAP))[[1]]
umap_curve_12d <- slingCurves(embedCurves(sshot_12d,
                                          reducedDims(d12_cells_sce)$UMAP))[[1]]
...
ctrl6d_idx <- which(d6_cells$treatment_class == "Control")
rocki6d_idx <- which(d6_cells$treatment_class == "ROCKi")
ctrl12d_idx <- which(d12_cells$treatment_class == "Control")
rocki12d_idx <- which(d12_cells$treatment_class == "ROCKi")
...
xmin_sep <- min(reducedDim(ctrl6d_cells_sce, "UMAP")[, 1],
                reducedDim(rocki6d_cells_sce, "UMAP")[, 1],
                reducedDim(ctrl12d_cells_sce, "UMAP")[, 1],
                reducedDim(rocki12d_cells_sce, "UMAP")[, 1])
xmax_sep <- max(reducedDim(ctrl6d_cells_sce, "UMAP")[, 1],
                reducedDim(rocki6d_cells_sce, "UMAP")[, 1],
                reducedDim(ctrl12d_cells_sce, "UMAP")[, 1],
                reducedDim(rocki12d_cells_sce, "UMAP")[, 1])
ymin_sep <- min(reducedDim(ctrl6d_cells_sce, "UMAP")[, 2],
                reducedDim(rocki6d_cells_sce, "UMAP")[, 2],
                reducedDim(ctrl12d_cells_sce, "UMAP")[, 2],
                reducedDim(rocki12d_cells_sce, "UMAP")[, 2])
ymax_sep <- max(reducedDim(ctrl6d_cells_sce, "UMAP")[, 2],
                reducedDim(rocki6d_cells_sce, "UMAP")[, 2],
                reducedDim(ctrl12d_cells_sce, "UMAP")[, 2],
                reducedDim(rocki12d_cells_sce, "UMAP")[, 2])
...
ctrl6d_traj_umap <- create_traj_umap(sshot_ctrl6d_separate, umap_curve_ctrl6d,
                                     Idents(ctrl6d_cells))
rocki6d_traj_umap <- create_traj_umap(sshot_rocki6d_separate,
                                      umap_curve_rocki6d, Idents(rocki6d_cells))
ctrl12d_traj_umap <- create_traj_umap(sshot_ctrl12d_separate,
                                      umap_curve_ctrl12d, Idents(ctrl12d_cells))
rocki12d_traj_umap <- create_traj_umap(sshot_rocki12d_separate,
                                       umap_curve_rocki12d,
                                       Idents(rocki12d_cells))

(ctrl6d_traj_umap_no_legend <- ctrl6d_traj_umap +
                               theme(legend.position = "none"))
(rocki6d_traj_umap_no_legend <- rocki6d_traj_umap +
                                theme(legend.position = "none"))
(ctrl12d_traj_umap_no_legend <- ctrl12d_traj_umap +
                                theme(legend.position = "none"))
(rocki12d_traj_umap_no_legend <- rocki12d_traj_umap +
                                 theme(legend.position = "none"))

if (save_images) {
    save_tiff(ctrl6d_traj_umap_no_legend, "fig_7a.tiff", width = 10, height = 8)
    save_tiff(rocki6d_traj_umap_no_legend, "fig_7b.tiff", width = 10,
              height = 8)
    save_tiff(ctrl12d_traj_umap_no_legend, "fig_7c.tiff", width = 10,
              height = 8)
    save_tiff(rocki12d_traj_umap_no_legend, "fig_7d.tiff", width = 10,
              height = 8)
}
```
...
```{r out.width = "100%"}
d6_cells$cell_type <- Idents(d6_cells)
d12_cells$cell_type <- Idents(d12_cells)
...
traj_df_ctrl6d <- create_traj_df(sshot_ctrl6d_together$slingPseudotime_1,
                                 "Control-6D",
                                 d6_cells[, ctrl6d_idx]$cell_type)
traj_df_rocki6d <- create_traj_df(sshot_rocki6d_together$slingPseudotime_1,
                                  "ROCKi-6D",
                                  d6_cells[, rocki6d_idx]$cell_type)
traj_df_6d <- rbind(traj_df_ctrl6d, traj_df_rocki6d)

traj_df_ctrl12d <- create_traj_df(sshot_ctrl12d_together$slingPseudotime_1,
                                  "Control-12D",
                                  d12_cells[, ctrl12d_idx]$cell_type)
traj_df_rocki12d <- create_traj_df(sshot_rocki12d_together$slingPseudotime_1,
                                   "ROCKi-12D",
                                   d12_cells[, rocki12d_idx]$cell_type)
traj_df_12d <- rbind(traj_df_ctrl12d, traj_df_rocki12d)

ptime_abs_diffs_6d <- abs(traj_df_ctrl6d$ptime - traj_df_rocki6d$ptime)
obs_result_ptime_6d <- mean(ptime_abs_diffs_6d)
ptime_abs_diffs_12d <- abs(traj_df_ctrl12d$ptime - traj_df_rocki12d$ptime)
obs_result_ptime_12d <- mean(ptime_abs_diffs_12d)
...
diffs_quants_traj6d <- quantile((bs_out$TrajDiff6D - obs_result_ptime_6d),
                                alpha_vals)
...
diffs_quants_traj12d <- quantile((bs_out$TrajDiff12D - obs_result_ptime_12d),
                                alpha_vals)
...
diffs_quants_speed6d <- quantile((bs_out$SpeedDiff6D - obs_result_speed_6d),
                                  alpha_vals)
...
diffs_quants_speed12d <- quantile((bs_out$SpeedDiff12D - obs_result_speed_12d),
                                  alpha_vals)
...
```{r}
skin_data_with_outliers <- skin_data_with_outliers %>%
                           SCTransform() %>%
                           CellCycleScoring(s.features = s_genes,
                                            g2m.features = g2m_genes)
skin_data_with_outliers$CC.difference <- skin_data_with_outliers$S.Score -
                                         skin_data_with_outliers$G2M.Score
skin_data_with_outliers <- skin_data_with_outliers %>%
                           SCTransform(vars.to.regress = c("CC.difference")) %>%
                           RunPCA(features = skin_data_with_outliers@assays$SCT@var.features,
                                  npcs = 50)
donor_idents <- sapply(skin_data_with_outliers$orig.ident,
                       function(x) {
                           if (x %in% c("1", "2", "3", "4")) {
                               "1"
                           } else if (x %in% c("5", "6", "7", "8")) {
...
skin_data_with_outliers$donor_idents <- donor_idents
skin_data_with_outliers <- skin_data_with_outliers %>%
                           RunHarmony("donor_idents", assay.use = "SCT") %>%
                           RunUMAP(dims = 1:50, reduction = "harmony")
p <- FeaturePlot(skin_data_with_outliers, "VIM")
if (save_images) {
...
```{r}
skin_data <- RunUMAP(skin_data, reduction = "harmony", dims = 1:50)
p <- FeaturePlot(skin_data, features = c("KRT14", "VIM"), blend = TRUE,
