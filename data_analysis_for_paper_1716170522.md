...
umap_d6_legend_plot <- ggplot(data.frame(class = as.character(1:length(labels)),
                                         x = 1:length(labels)),
...
umap_d6_legend <- as_ggplot(get_legend(umap_d6_legend_plot, "left"))

plot_treatment_class_umap <- function(cells, umap_lims) {
...
(umap_d6 <- plot_treatment_class_umap(d6_cells, umap_lims) +
...
(umap_d12 <- plot_treatment_class_umap(d12_cells, umap_lims) +
...
create_daseq_umap <- function(daseq_out, umap_lims) {
    daseq_legend <- as_ggplot(get_legend(daseq_out$pred.plot +
                                         umap_theme +
                                         theme(legend.position = "top",
                                               legend.text = element_text(size = 8))))
    daseq_umap <- daseq_out$pred.plot + umap_lims + umap_theme + theme(legend.position = "none")
    out_umap <- daseq_umap + annotation_custom(ggplotGrob(daseq_legend),
                                               xmin = umap_lims$limits$x[2] - 5,
                                               xmax = umap_lims$limits$x[2] - 4,
                                               ymin = umap_lims$limits$y[1],
                                               ymax = umap_lims$limits$y[1] + 1)

    return(out_umap)
}

(daseq_umap_6d <- create_daseq_umap(daseq_out_6d, umap_lims) +
...
(daseq_umap_12d <- create_daseq_umap(daseq_out_12d, umap_lims) +
...
vol_6d <- VolPlot(marks_6d) + ggtitle("Differentially Expressed Genes at 6D")

labels <- c(bquote(Log["2"]~"fold change ≤ -1"),
            bquote("|"*Log["2"]~"fold change| < 1"),
            bquote(Log["2"]~"fold change ≥ 1"))
...
vol_6d_legend <- as_ggplot(get_legend(vol_6d_legend_plot, "right"))
(vol_6d_with_legend <- vol_6d + annotation_custom(ggplotGrob(vol_6d_legend),
                                                 xmin = 1, ymin = 200))
```
...
# tiff("paper_figs/fig_5f.tiff", units="cm", width=10, height=5, res=300); vol_12d; dev.off()
```
...
# tiff("paper_figs/fig_6b.tiff", units="cm", width=10, height=8, res=300); cell_type_umap_ctrl12d; dev.off()
# tiff("paper_figs/fig_6c.tiff", units="cm", width=10, height=8, res=300); cell_type_umap_rocki6d; dev.off()
# tiff("paper_figs/fig_6d.tiff", units="cm", width=10, height=8, res=300); cell_type_umap_rocki12d; dev.off()
...
```r
# tiff("paper_figs/fig_6g.tiff", units="cm", width=20, height=8, res=300); holo_dp; dev.off()
# tiff("paper_figs/fig_s2a.tiff", units="cm", width=20, height=8, res=300); ta_dp; dev.off()
# tiff("paper_figs/fig_s2b.tiff", units="cm", width=20, height=8, res=300); td_dp; dev.off()
```
...
# tiff("paper_figs/fig_7b.tiff", units="cm", width=10, height=8, res=300); rocki6d_traj_umap_no_legend; dev.off()
# tiff("paper_figs/fig_7c.tiff", units="cm", width=10, height=8, res=300); ctrl12d_traj_umap_no_legend; dev.off()
# tiff("paper_figs/fig_7d.tiff", units="cm", width=10, height=8, res=300); rocki12d_traj_umap_no_legend; dev.off()
```
...
# tiff("paper_figs/fig_7f.tiff", units="cm", width=10, height=8, res=300); traj_plot_12d_no_legend; dev.off()
...
# tiff("paper_figs/fig_s3b.tiff", units="cm", width=14, height=8, res=300); speed_plot_12d; dev.off()
