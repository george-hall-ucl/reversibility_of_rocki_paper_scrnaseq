...
prepare_cells_for_slingshot <- function(cells) {
    ...
    if (cells@meta.data$sample_class == "Control-6D") {
...
skin_data.with_outliers <- skin_data.with_outliers %>% SCTransform() %>%
                           CellCycleScoring(s.features = s.genes,
                                            g2m.features = g2m.genes)
skin_data.with_outliers@meta.data$CC.difference <- skin_data.with_outliers@meta.data$S.Score - skin_data.with_outliers@meta.data$G2M.Score
skin_data.with_outliers <- skin_data.with_outliers %>%
                           ...
                           RunPCA(features = skin_data.with_outliers@assays$SCT@var.features,
                                  npcs = 50)
skin_data.with_outliers@meta.data$donor_idents <- sapply(skin_data.with_outliers@meta.data$orig.ident,
                                                         function(x) if (x %in% c("1","2","3","4")) {"1"}
                                                         else if (x %in% c("5","6","7","8")) {"2"}
                                                         else {"3"})
skin_data.with_outliers <- skin_data.with_outliers %>%
