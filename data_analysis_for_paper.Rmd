...
cells$d6 <- RunUMAP(cells$d6, dims = 1:50, reduction = "harmony")
cells$d12 <- RunUMAP(cells$d12, dims = 1:50, reduction = "harmony")
...
(umap_d6 <- plot_treatment_class_umap(cells$d6))
if (save_images) {
    save_tiff(umap_d6, "fig_5a.tiff")
}

(umap_d12 <- plot_treatment_class_umap(cells$d12))
if (save_images) {
...
```{r}
daseq_out_6d <- getDAcells(Embeddings(object = cells$d6, reduction = "harmony"),
                           cell.labels = as.character(cells$d6$sample_class),
                           labels.1 = c("Control-6D"),
                           labels.2 = c("ROCKi-6D"),
                           plot.embedding = Embeddings(object = cells$d6,
                                                       reduction = "umap"))

daseq_out_12d <- getDAcells(Embeddings(object = cells$d12,
                                       reduction = "harmony"),
                            cell.labels = as.character(cells$d12$sample_class),
                            labels.1 = c("Control-12D"),
                            labels.2 = c("ROCKi-12D"),
                            plot.embedding = Embeddings(object = cells$d12,
                                                        reduction = "umap"))
...
proportion_da_6d <- num_da_cells_6d / length(colnames(cells$d6))
num_da_cells_12d <- length(c(daseq_out_12d$da.down, daseq_out_12d$da.up))
proportion_da_12d <- num_da_cells_12d / length(colnames(cells$d12))
```
...
marks_6d <- FindMarkers(cells$all, ident.1 = "ROCKi-6D",
                        ident.2 = "Control-6D")
marks_12d <- FindMarkers(cells$all, ident.1 = "ROCKi-12D",
                         ident.2 = "Control-12D")
...
cells$all$cell_type_pred <- SCINA(as.matrix(GetAssayData(cells$all)),
                                  signatures)$cell_labels
cells$d6$cell_type_pred <- SCINA(as.matrix(GetAssayData(cells$d6)),
                                 signatures)$cell_labels
cells$d12$cell_type_pred <- SCINA(as.matrix(GetAssayData(cells$d12)),
                                  signatures)$cell_labels

cells$d6$cell_type_pred <- factor(cells$d6$cell_type_pred,
                                  levels = c("Basal", "TA", "TD", "unknown"))
cells$d12$cell_type_pred <- factor(cells$d12$cell_type_pred,
                                   levels = c("Basal", "TA", "TD", "unknown"))
...
cell_type_umap_ctrl6d <- create_cell_type_umap(cells$ctrl6d)
cell_type_umap_rocki6d <- create_cell_type_umap(cells$rocki6d)
cell_type_umap_ctrl12d <- create_cell_type_umap(cells$ctrl12d)
cell_type_umap_rocki12d <- create_cell_type_umap(cells$rocki12d)
...
cell_type_df_6d <- data.frame(CellType = cells$d6$cell_type_pred,
                           Class = cells$d6$sample_class)
...
cell_type_df_12d <- data.frame(CellType = cells$d12$cell_type_pred,
                           Class = cells$d12$sample_class)
...
obs_total_prop_diff_6d <- calc_cell_type_props(cells$d6$cell_type_pred,
                                               cells$d6$treatment_class)
obs_total_prop_diff_12d <- calc_cell_type_props(cells$d12$cell_type_pred,
                                                cells$d12$treatment_class)
...
for (r in 1:10000) {
    ...
    c6d_idx <- sample(which(cells$d6$treatment_class == "Control"),
                      length(which(cells$d6$treatment_class == "Control")),
                      replace = TRUE)
    r6d_idx <- sample(which(cells$d6$treatment_class == "ROCKi"),
                      length(which(cells$d6$treatment_class == "ROCKi")) - 1,
                      replace = TRUE)
    d6_idx <- c(c6d_idx, r6d_idx)

    c12d_idx <- sample(which(cells$d12$treatment_class == "Control"),
                      length(which(cells$d12$treatment_class == "Control")),
                      replace = TRUE)
    r12d_idx <- sample(which(cells$d12$treatment_class == "ROCKi"),
                      length(which(cells$d12$treatment_class == "ROCKi")) - 1,
                      replace = TRUE)
    d12_idx <- c(c12d_idx, r12d_idx)

    # Find cell type and treatment class labels for this resample
    d6_cell_type <- cells$d6$cell_type_pred[d6_idx]
    d6_treat_class <- cells$d6$treatment_class[d6_idx]

    d12_cell_type <- cells$d12$cell_type_pred[d12_idx]
    d12_treat_class <- cells$d12$treatment_class[d12_idx]
...
levels(cells$all$sample_class_space) <- c("Control-6D ",
                                                    "ROCKi-6D ",
                                                    "Control-6D 6D ",
                                                    "ROCKi-6D 6D ")

holo_cells <- subset(cells$all, subset = cell_type_pred == "Basal")
ta_cells <- subset(cells$all, subset = cell_type_pred == "TA")
td_cells <- subset(cells$all, subset = cell_type_pred == "TD")
...
cells$ctrl6d <- RunUMAP(cells$ctrl6d, dims = 1:50, reduction = "harmony")
cells$rocki6d <- RunUMAP(cells$rocki6d, dims = 1:50, reduction = "harmony")
cells$ctrl12d <- RunUMAP(cells$ctrl12d, dims = 1:50, reduction = "harmony")
cells$rocki12d <- RunUMAP(cells$rocki12d, dims = 1:50, reduction = "harmony")
...
cells$d6 <- prepare_cells_for_slingshot(cells$d6)
cells$d12 <- prepare_cells_for_slingshot(cells$d12)
cells$ctrl6d <- prepare_cells_for_slingshot(cells$ctrl6d)
cells$rocki6d <- prepare_cells_for_slingshot(cells$rocki6d)
cells$ctrl12d <- prepare_cells_for_slingshot(cells$ctrl12d)
cells$rocki12d <- prepare_cells_for_slingshot(cells$rocki12d)
```
...
d6_cells_sce <- as.SingleCellExperiment(cells$d6)
d12_cells_sce <- as.SingleCellExperiment(cells$d12)
ctrl6d_cells_sce <- as.SingleCellExperiment(cells$ctrl6d)
cells$rocki6d_sce <- as.SingleCellExperiment(cells$rocki6d)
cells$ctrl12d_sce <- as.SingleCellExperiment(cells$ctrl12d)
cells$rocki12d_sce <- as.SingleCellExperiment(cells$rocki12d)
...
reducedDim(cells$rocki6d_sce, "UMAP")[, 2] <- (-1) * reducedDims(cells$rocki6d_sce)$UMAP[, 2]
reducedDim(cells$ctrl12d_sce, "UMAP")[, 1] <- (-1) * reducedDims(cells$ctrl12d_sce)$UMAP[, 1]

sshot_6d <- slingshot(d6_cells_sce, Idents(cells$d6),
                      reducedDim = "HARMONY", start.clus = "Basal")
sshot_12d <- slingshot(d12_cells_sce, Idents(cells$d12),
                       reducedDim = "HARMONY", start.clus = "Basal")

sshot_ctrl6d_separate <- slingshot(ctrl6d_cells_sce,
                                   Idents(cells$ctrl6d),
                                   reducedDim = "UMAP", start.clus = "Basal")
sshot_rocki6d_separate <- slingshot(cells$rocki6d_sce,
                                    Idents(cells$rocki6d),
                                    reducedDim = "UMAP", start.clus = "Basal")
sshot_ctrl12d_separate <- slingshot(cells$ctrl12d_sce,
                                    Idents(cells$ctrl12d),
                                    reducedDim = "UMAP", start.clus = "Basal")
sshot_rocki12d_separate <- slingshot(cells$rocki12d_sce,
                                     Idents(cells$rocki12d),
                                     reducedDim = "UMAP", start.clus = "Basal")
...
ctrl6d_idx <- which(cells$d6$treatment_class == "Control")
rocki6d_idx <- which(cells$d6$treatment_class == "ROCKi")
ctrl12d_idx <- which(cells$d12$treatment_class == "Control")
rocki12d_idx <- which(cells$d12$treatment_class == "ROCKi")
...
rocki6d_traj_umap <- create_traj_umap(sshot_rocki6d_separate,
                                      umap_curve_rocki6d, Idents(cells$rocki6d))
ctrl12d_traj_umap <- create_traj_umap(sshot_ctrl12d_separate,
                                      umap_curve_ctrl12d, Idents(cells$ctrl12d))
...
```{r out.width = "100%"}
cells$d6$cell_type <- Idents(cells$d6)
cells$d12$cell_type <- Idents(cells$d12)
